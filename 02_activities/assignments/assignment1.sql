...
SELECT 
vba.vendor_id, vendor_name,
count(vba.vendor_id) as number_times_booth_rented
...
SELECT
c.customer_id, customer_first_name, customer_last_name, sum(cost_to_customer_per_qty*quantity) as total_spent
...
and year are! */
SELECT
customer_id, strftime('%m', market_date) as month_of_purchase, strftime('%Y', market_date) as year_of_purchase,
product_id, vendor_id, quantity, cost_to_customer_per_qty
FROM customer_purchases;
...
SELECT
customer_id, sum(quantity*cost_to_customer_per_qty) as total_spent_April
--FROM customer_purchases
--WHERE market_date like '2022-04-%'
FROM my_purchase
WHERE month_of_purchase = '04' AND year_of_purchase = '2022'
